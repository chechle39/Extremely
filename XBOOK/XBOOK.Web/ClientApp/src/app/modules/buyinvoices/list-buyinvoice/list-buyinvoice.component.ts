...
export class ListBuyInvoiceComponent extends PagedListingComponentBase<InvoiceView> {
  ...
  redirectToCreateNewInvoice() {
    this.router.navigate([`/buyinvoice/new`]);
  }
  redirectToEditInvoice(id) {
    this.router.navigate([`/buyinvoice/${id}/${ActionType.Edit}`]);
  }
  ...
  onActivate(event) {
    ...
    if (event.type === 'click') {
      if (event.cellIndex > 0 && this.isCheckBackTo === false) {
        this.router.navigate([`/buyinvoice/${event.row.invoiceId}/${ActionType.View}`]);
      }
    }
  }
  sortClient() {
