...
@Injectable()
export class GenLedService extends BaseService {
  searchGen(request: any) {
    const gen = this.post<any[]>(`${API_URI.getGen}`, request)
      .pipe(
  ...
  GenSaveDataPrint(requeData: any) {
    return this.post(`${API_URI.GenLedSaveDataPrint}`, requeData);
  }
