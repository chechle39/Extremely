...
export class CreateJournalComponent extends AppComponentBase implements OnInit {
  ...
  onBlurMethod() {
    this.invoiceForm.controls.entryName.value;
    this.invoiceForm.controls.description.patchValue(this.invoiceForm.controls.entryName.value);
  }
  createForm() {
  ...
  save() {
    ...
    if (this.invoiceId === 0) {
      const request = {
        ...
        description: this.invoiceForm.value.description === this.invoiceForm.value.entryName ? 'Entry Description' : this.invoiceForm.value.description,
      ...
      this.journalEntryService.createJournal(request).subscribe(rp => {
        ...
        this.close(true);
        // this.invoiceForm.reset();
        // for (let i = 0; i < this.invoiceForm.value.items.length - 1; i++) {
        //   this.removeItem(i);
        // }
        // // this.viewMode = false;
