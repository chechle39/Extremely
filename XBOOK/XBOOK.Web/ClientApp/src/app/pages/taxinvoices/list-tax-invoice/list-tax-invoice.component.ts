...
@Component({
  selector: 'xb-list-tax-invoice',
  styleUrls: ['./list-tax-invoice.component.scss'],
  templateUrl: './list-tax-invoice.component.html',
})

export class ListTaxInvoiceComponent extends PagedListingComponentBase<TaxInvoiceView> implements OnDestroy {
  ...
  protected list(
    ...
    if (this.router.url !== '/pages/taxinvoice') {
      this.invoiceOfChart();
    } else {
  ...
  coppy() {
    ...
    this.router.navigate([`pages/taxinvoice/${this.selected[0].taxInvoiceId}/${ActionType.Coppy}`]);
  }
  ...
  redirectToCreateNewInvoice() {
    this.router.navigate([`pages/taxinvoice/new`]);
  }
  redirectToEditInvoice(id) {
    this.data.sendMessage('');
    this.router.navigate([`pages/taxinvoice/${id}/${ActionType.Edit}`]);
  }
  ...
  addPayment() {
    ...
    for (let i = 0; i < this.selected.length; i++) {
      ...
      if (amountDue > 0) {
        data.push(amountDue);
        const invoice = {
          taxInvoiceId: this.selected[i].taxInvoiceId,
          dueDate: this.selected[i].dueDate,
          amountIv: amountDue,
        };
        taxInvoiceId.push(invoice);
      }
    }
    ...
    forkJoin(
      ...
      dialog.componentInstance.taxInvoiceId = taxInvoiceId;
  ...
  onActivate(event) {
    ...
    if (event.type === 'click') {
      if (event.cellIndex > 0 && this.isCheckBackTo === false) {
        this.router.navigate([`pages/taxinvoice/${event.row.taxInvoiceId}/${ActionType.View}`]);
      }
    }
  }
  sortClient() {
